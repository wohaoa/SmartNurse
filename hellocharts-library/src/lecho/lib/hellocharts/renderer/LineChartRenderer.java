...
public class LineChartRenderer extends AbstractChartRenderer {
	...
	private static final int MODE_ONLY = 2;
	...
	public void drawUnclipped(Canvas canvas) {
		...
		for (Line line : data.getLines()) {
			...
			} else if (checkIfShouldDrawOnePoint(line)) {
				drawPoints(canvas, line, lineIndex, MODE_ONLY);
			}
			++lineIndex;
		}
		if (isTouched()) {
	...
	private boolean checkIfShouldDrawOnePoint(Line line) {
		return !line.hasPoints();
	}
	...
	private void drawPoints(Canvas canvas, Line line, int lineIndex, int mode) {
		...
		for (PointValue pointValue : line.getValues()) {
			...
			if (computator.isWithinContentRect(rawX, rawY, checkPrecision)) {
				...
				} else if (MODE_ONLY == mode) {
					if (line.hasLabels()) {
						if (valueIndex == line.getValues().size() - 1) {
							drawPoint(canvas, line, pointValue, rawX, rawY, pointRadius);
							drawLabel(canvas, line, pointValue, rawX + 120, rawY + 50, pointRadius + labelOffset);
						}
					}
