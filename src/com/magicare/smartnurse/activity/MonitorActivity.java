...
public class MonitorActivity extends BaseActivity implements ResolveListener, FeedListener, UserDetailListener {
	...
	private String IMEI;
	private int pension_areaid;
	private boolean mOnlyWarning = false; // 为true表示只显示报警列表
	...
	public void onResume() {
		...
		mHandler.sendEmptyMessageDelayed(REFRESH_WARNNING, 1 * 1000);
		
		// 刷新用户状态信息
		mHandler.removeMessages(REFRESH_USERSTATUS);
		mHandler.sendEmptyMessageDelayed(REFRESH_USERSTATUS, 1 * 1000);

		if (ConfigManager.getStringValue(this.getApplicationContext(), ConfigManager.IMEI) != null
				&& !ConfigManager.getStringValue(this.getApplicationContext(), ConfigManager.IMEI).equals("")) {
			IMEI = ConfigManager.getStringValue(this.getApplicationContext(), ConfigManager.IMEI);
		} else {
			TelephonyManager TelephonyMgr = (TelephonyManager) this.getSystemService(this.TELEPHONY_SERVICE);
			IMEI = TelephonyMgr.getDeviceId();
			ConfigManager.setStringValue(this.getApplicationContext(), ConfigManager.IMEI, IMEI);
		}
	}
	...
	public void getWarning(final Context context) {
		...
		client.getAlarmInfoByStatus(context, ConfigManager.getStringValue(context, Constants.ACCESS_TOKEN),
				pension_areaid, "2", new IOperationResult() {
						...
						if (isSuccess) {
							if (TextUtils.isEmpty(json) || !json.startsWith("{")) {
								...
								if (baseBean.getStatus() == 0) {
									...
									PromptManager.closeProgressDialog(); // 关闭加载的进度条
									
									if(!mOnlyWarning){
										gv_dynamic.setVisibility(View.VISIBLE);
									...
									PromptManager.showToast(mContext, false, errors + " " + baseBean.getInfo());									
								}
							}
	...
	private void getUserStatusInfo() { // 可筛选出所有在该区域有警告的老人
		...
		client.getUserStatusInfo(mContext, ConfigManager.getStringValue(mContext, Constants.ACCESS_TOKEN),
				pension_areaid, new IOperationResult() {
	...
	private void initview() {
		pension_areaid = getIntent().getIntExtra("pension_areaid", 0);
		mOnlyWarning = getIntent().getBooleanExtra("onlyWarning", false);
		fl_main = (FrameLayout) findViewById(R.id.fl_main);
		lv_warning = (ListView) findViewById(R.id.lv_warning);
		mLayoutDynamic = (LinearLayout) findViewById(R.id.layout_dynamic);

		// girdview头像列表
		gv_dynamic = (GridView) findViewById(R.id.gv_dynamic);
		...
		tv_dynamic_title = (TextView) findViewById(R.id.tv_dynamic_title);
		...
		// 告警列表的adapter
		mWarningAdapter = new WarningDealWithAdapter(mContext, list_warning, false);
		mWarningAdapter.setOnResolveListener(this);
		mWarningAdapter.setOnFeedListener(this);
		mWarningAdapter.setOnUserDetailListener(this);
		...
		// 加载完数据之前先默认隐藏
		gv_dynamic.setVisibility(View.GONE);
