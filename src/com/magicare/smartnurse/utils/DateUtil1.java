...
public class DateUtil1 {
	...
	public static List<HealthBean> getListHealth(List<HealthBean> beans)
		...
		for (int i = 0; i < map.size(); i++) {
			...
			for (int j = 0; j < beans3.size(); j++) {
				...
				if (j == 0) {
					...
					if (bean.getDiastolic_pressure() > 0) {
						counts[4] = counts[4] + 1;
						diastolic_pressure += bean.getDiastolic_pressure();
					}
				} else {
			...
			bean2.setDiastolic_pressure(((float) diastolic_pressure)
					/ (counts[4] == 0 ? 1 : counts[4]));
			beans2.add(bean2);
		}
		...
		// for (HealthBean healthBean : beans3) {
		// System.out.println("beans3" + healthBean);
		// }
		...
		// for (HealthBean healthBean : beans4) {
		// System.out.println("最后"+healthBean);
		// }
	...
	public static long getMinuteDiff(String date, String date1)
		...
		java.util.Date begin = dfs.parse(date);
		java.util.Date end = dfs.parse(date1);
		long between = (end.getTime() - begin.getTime()) / 1000;//秒
	...
	public static String getDatePerviousAndNext(String date, int posion)
		...
		Calendar c = Calendar.getInstance();
		c.set(begin.getYear() + 1900, begin.getMonth() + 1, begin.getDate());
