...
public class AsyncImageLoader {
    ...
    public AsyncImageLoader(Context context, ListView listview, List<ConcernBean> list) {
        this.context = context;
    	this.imgListView = listview;
    	this.mList = list;
        initMemCache();
        initDiskLruCache();
    }
     ...
     * @param imageUrl
     */
    public Bitmap loadImage(ImageView imageView, String imageUrl, int id) {
        // 先从内存中拿
        ...
        if (!TextUtils.isEmpty(imageUrl)) {
            new ImageDownloadTask(imageView, id).execute(imageUrl);
        }
    ...
    public void loadImage(int firstVisibleItem, int visibleItemCount) {
    	for (int i = firstVisibleItem; i < firstVisibleItem + visibleItemCount; i++) {
			...
			if (bitmap == null) {//缓存中没有，先从拷贝的number路径中取
		        ...
		        if (bitmap == null) {
		            if (!TextUtils.isEmpty(imageUrl)) {
		            	ImageView imgView = (ImageView) imgListView.findViewWithTag(mList.get(i).getExhort_id());
						if (imgView != null) {
//							imgView.setImageBitmap(bitmap);
							new ImageDownloadTask(imgView, mList.get(i).getExhort_id()).execute(imageUrl);
						}
//		                new ImageDownloadTask(imgView).execute(imageUrl);
		            }
		        }else{
		            putBitmapToMem(imageUrl, bitmap);
		            ImageView imgView = (ImageView) imgListView.findViewWithTag(mList.get(i).getExhort_id());
    ...
    class ImageDownloadTask extends AsyncTask<String, Integer, Bitmap> {
        ...
        public ImageDownloadTask(ImageView imageView, int id) {
